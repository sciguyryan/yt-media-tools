...
- Continue the scalar-expression expansion with `CASE`, `LIKE`/`NOT LIKE`, `ILIKE`/`NOT ILIKE`, `GREATEST`, `LEAST`, `NULLIF`, useful date extraction functions, and a deliberately defined `SELECT *` contract. General arithmetic, nested scalar functions and expression-based `ORDER BY` are implemented.
