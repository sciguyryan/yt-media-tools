...
- Continue the scalar-expression expansion with useful date extraction functions and a deliberately defined `SELECT *` contract. General arithmetic, nested scalar functions, `NULLIF`, `GREATEST`, `LEAST`, expression-based `ORDER BY`, and searched `CASE` are implemented.
