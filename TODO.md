...
- Continue extending the implemented `UNION` and `UNION ALL` architecture only where composition semantics remain useful to Discover. Do not add relational database features such as `JOIN`, DML, DDL, transactions, indexes, procedures or triggers merely for SQL familiarity when they do not fit Discover's acquisition/query model.
