...
- Continue the scalar-expression expansion with `LIKE`/`NOT LIKE`, `ILIKE`/`NOT ILIKE`, `GREATEST`, `LEAST`, `NULLIF`, useful date extraction functions, and a deliberately defined `SELECT *` contract. General arithmetic, nested scalar functions, expression-based `ORDER BY`, and searched `CASE` are implemented.
