...
- Add `RANDOM()` in its own dedicated dot release after UNION composition hardening. Support volatile unseeded randomness and deterministic seeded randomness. Permit projection as a synthetic scalar column and ordering by the projected alias, but define explicit placement restrictions rather than allowing volatile randomness indiscriminately in predicates, grouping or aggregate filters. The optimiser must never constant-fold volatile randomness, seeded values should derive reproducibly from the seed plus stable logical row identity rather than evaluation order, and random ordering must disable source-order early LIMIT termination.
- Continue hardening the implemented `UNION` and `UNION ALL` architecture over compatible logical query-result schemas. Do not add relational database features such as `JOIN`, DML, DDL, transactions, indexes, procedures or triggers merely for SQL familiarity when they do not fit Discover's acquisition/query model.
- Extend the deterministic multi-source fixtures introduced with `UNION` as composition hardening continues. The current suite includes channel-like, playlist-like and Twitch-like source shapes with missing metadata, NULLs, incompatible dynamic kinds, Unicode and duplicate elimination; add broader aggregation, provenance and scale cases where useful.
