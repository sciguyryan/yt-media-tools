...
- Freeze new syntactic sugar now that the `OF` source/facet series is complete. Do not add further grammar features until the cleanup, optimiser-audit and completeness-review programme has passed local acceptance.
- Complete the post-0.26.2 hardening programme in deliberate phases. The initial dead-code and stale-state sweep is implemented in 0.26.3; next audit and improve safe optimiser strategies, then perform the exhaustive completeness review and brutal torture suite before considering more language syntax.
- The torture selection must cover deeply nested expressions, hostile whitespace, malformed clauses, keyword ambiguity, Unicode boundaries, comments or unsupported tokens where relevant, CTE/UNION composition, source/facet identity, RANDOM semantics, NULL/three-valued logic, precedence, numeric literal forms, temporal literals, aggregate syntax, deterministic error locations and optimised-versus-unoptimised equivalence.
