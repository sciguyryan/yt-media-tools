...
Relations retain source/facet identity and, once a JOIN is present, semantic resolution requires an explicit `AS` alias for every participating relation. Joined relations are represented separately from the primary `FROM` relation, in source order, with each join carrying its own `ON` predicate. Qualified names such as `a.title` remain syntactically compatible with existing dotted field syntax at the parser boundary; semantic resolution distinguishes relation qualification from structured member access, records relation-owned fields explicitly, and rejects duplicate or unknown aliases and ambiguous unqualified fields deterministically.

The grammar was deliberately staged ahead of execution. Single `INNER`, `LEFT [OUTER]`, `SEMI` and `ANTI` JOIN execution is now implemented and validated across acquisition, cardinality, NULL extension, provenance, optimisation and evaluation. Multi-way JOIN remains behind a deterministic fail-closed execution boundary rather than partially executing as a single-relation query.

Projection semantics are now defined for staged JOIN queries. Plain `*` expands only the primary/left relation and retains its established stand-alone rule. `alias.*` expands only the explicitly named relation and may be combined with other projections. Right-side fields are never flattened implicitly, and duplicate projected output names are rejected unless explicit `AS` aliases make them unique. These rules are resolved before executable JOIN evaluation and remain part of the deterministic relation contract.
