...
Issues #102, #103, #104, #109, #110, #111 and #112 used this shared harness to evaluate lightweight known-video metadata acquisition. The investigation ultimately promoted bounded YouTube.js `getBasicInfo()` and specialised ytmusicapi acquisition into production through issues #114 and #116; `youtube-innertube`, pytubefix, NewPipeExtractor, Invidious and Piped remain research-only providers. The harness is retained so deferred decisions can be revisited against the same provider-neutral corpus when materially new evidence appears.
...
The reconciled research provider therefore uses the explicit current day-based signature timestamp by default, matching the accepted anonymous production requirement from issue #116. `--ytmusicapi-library-default-signature` retains the former library-default behaviour only as a diagnostic comparison. `--ytmusicapi-auth FILE` remains available for controlled research but is not part of the production path. The authentication file contains credential material and must not be committed, packaged, pasted into benchmark output or supplied through external-invocation diagnostics.
