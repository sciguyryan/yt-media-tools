...
The follow-up benchmark showed that the parsed YouTube.js `VideoInfo` microformat is also null for the observed legacy upload. To distinguish parser exposure from response omission without another browser-source exercise, the benchmark now inspects a clone of the raw `/youtubei/v1/player` response in memory and retains only whether `playerMicroformatRenderer` was present plus its publication, upload and live-broadcast fields. The raw response body is not retained. This diagnostic capture is reset for every benchmark video and remains research evidence rather than a production metadata source.

A null normalised benchmark field therefore remains distinct from both a null parsed microformat field and absence in the raw InnerTube response. Production lowering remains limited to the five exact scalar fields already established for issue #114.
