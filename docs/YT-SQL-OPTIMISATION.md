...
Collection-query expressions retain operation-specific acquisition requirements for `ANY`, `ALL`, `CARDINALITY`, scoped `COUNT`, `FILTER` and `MAP`. Bound element references remain local to the collection scope, while ordinary row fields referenced from inside the scoped expression are recorded as outer dependencies. Pushdown is legal only when every required operation on the physical collection has an explicit exact capability and the operation is uncorrelated. The exact capability contract includes yt-sql NULL handling, three-valued predicate reduction, logical ordering and result typing. Missing proof, mixed supported/unsupported composed operations, whole-collection conflicts or outer-row correlation widen conservatively to complete collection acquisition and local evaluation. Current adapters advertise no exact collection-query pushdown capabilities.
...
The shared predicate prover currently establishes resolved constant truth and a deliberately bounded family of never-TRUE filtering contradictions. The constraint vocabulary includes incompatible equality and inequality requirements, reversed literal comparisons, inclusive positive BETWEEN bounds, finite positive IN-domain intersections, incompatible NULL requirements, and recursively composed Boolean branches. AND is never TRUE when any constituent is independently proven never TRUE or when its combined same-field constraints are contradictory. OR is never TRUE only when every branch is independently proven never TRUE; one unresolved or potentially live branch prevents that proof. Relation simplification consumes these proofs for WHERE and HAVING. JOIN planning consumes the same never-TRUE proof and separately applies JOIN semantics: INNER and SEMI are then proven empty, while LEFT and ANTI retain the primary relation but prove the joined relation irrelevant. Acquisition planning consumes only those derived relational consequences. This separation keeps semantic facts, relational consequences and physical acquisition policy distinct.
...
The proof model is intentionally incomplete. OR composition is limited to the sound rule that every branch must independently be proven never TRUE; distributive rewriting, arbitrary Boolean algebra, inferred functional dependencies, speculative type/domain assumptions, volatile-expression equivalence and cross-relation transformations remain unproven unless a dedicated rule establishes their full yt-sql semantics. Finite-domain reasoning is currently limited to explicit positive IN lists and direct literal constraints; it does not infer undocumented field domains. Optimiser/reference differential tests and explicit non-proof tests are required as the proof surface expands.

Relation-level facts are likewise deliberately bounded. A CTE proven empty can prove a consuming primary relation empty; an empty right CTE proves INNER and SEMI empty but does not make LEFT or ANTI empty; and a UNION result is proven empty only when its primary body and every set branch are independently proven empty. An explicit LIMIT supplies an upper cardinality bound, while a proven-empty result has cardinality zero. Direct physical-source cardinality remains unknown.

The source/facet capability model is an authoritative source of stable input-field facts. The shared prover may consume its declared logical kind, complete `QueryType`, top-level nullability, structural-support state and collection-ordering contract. For example, `id IS NULL` is never TRUE where the selected capability declares `id` non-null, and `tags` may carry its declared stable collection ordering. These facts stop at the capability boundary: dynamic/provider-specific fields have no stable type, nullability or structural-support proof, and runtime provider ordering is never promoted into a logical collection-ordering guarantee. Type compatibility remains a resolver responsibility rather than a reason for the prover to reinterpret an invalid comparison as an impossible predicate.

Result-column fact propagation is deliberately narrower than source-field provability. The resolved AST retains enough provenance to lift authoritative source facts through a simple single-source identity projection, including an output alias, because the result expression still names the exact source field. Computed expressions, CTE boundaries, set reconciliation, grouped results and JOIN-owned fields remain proof boundaries because the current result-schema representation does not preserve enough expression-level provenance to carry those facts generally. LEFT JOIN is especially significant because NULL extension can make a non-null source field nullable in the result. Multi-way JOIN execution also remains a guarded boundary and is not used as a source of relation proofs. Physical-source cardinality is not declared by the capability model and remains unknown. These are explicit absences of authoritative semantic information, not invitations to infer from field names, observed datasets or extractor behaviour.
