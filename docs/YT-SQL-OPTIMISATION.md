...
Arbitrary result-column nullability and type propagation remain separate proof boundaries. The current CTE/result-schema construction intentionally does not retain enough expression-level provenance to lift every source declaration safely through projection, aliases, set reconciliation and LEFT JOIN NULL extension. Multi-way JOIN execution also remains a guarded boundary and is not used as a source of relation proofs. Physical-source cardinality is not declared by the capability model and remains unknown. These are explicit absences of authoritative semantic information, not invitations to infer from field names, observed datasets or extractor behaviour.
