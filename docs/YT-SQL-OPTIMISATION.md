...
Relation-level facts are likewise deliberately bounded. A CTE proven empty can prove a consuming primary relation empty; an empty right CTE proves INNER and SEMI empty but does not make LEFT or ANTI empty; and a UNION result is proven empty only when its primary body and every set branch are independently proven empty. An explicit LIMIT supplies an upper cardinality bound, while a proven-empty result has cardinality zero. Direct physical-source cardinality remains unknown. The source/facet capability model does provide authoritative nullability for stable logical input fields, so the shared prover may consume that contract, for example to prove `id IS NULL` never TRUE where `id` is declared non-null. Dynamic fields retain unknown nullability. Arbitrary result-column nullability remains a separate proof boundary because projection, CTE output mapping and LEFT JOIN null extension are not represented strongly enough to lift source declarations to every result column safely. Multi-way JOIN execution also remains a guarded boundary and is not used as a source of relation proofs.
