...
`UNION` remains deliberately positional. Selected JOIN syntax and AST structure are also reserved. A single `INNER JOIN`, `LEFT [OUTER] JOIN`, `SEMI JOIN` or `ANTI JOIN` is executable over independently acquired relations. `INNER JOIN` exposes both sides and preserves duplicate row-pair matches. `LEFT JOIN` additionally preserves unmatched primary rows with a NULL-extended right relation. Existence joins retain their non-multiplying filter semantics. Multi-way JOIN execution remains unsupported. JOIN resolution validates each `ON` predicate against only the relations visible at that join edge and rejects aggregate or volatile `RANDOM` expressions from row-level join matching. JOIN inputs may be direct source/facet relations or CTE-produced relations, and JOIN may appear independently in UNION/UNION ALL branches. Acquisition planning keeps each physical relation boundary distinct and assigns qualified field requirements to the source/facet that owns them. Source/facet selection continues to use the extractor-agnostic `OF` model described below.
