...
`UNION` remains deliberately positional. Selected JOIN syntax and AST structure are also reserved. A single `INNER JOIN`, `SEMI JOIN` or `ANTI JOIN` is executable over independently acquired relations. `INNER JOIN` exposes both sides and preserves duplicate row-pair matches; existence joins retain their non-multiplying filter semantics. `LEFT JOIN` and multi-way JOIN execution remain unsupported. JOIN resolution validates each `ON` predicate against only the relations visible at that join edge and rejects aggregate or volatile `RANDOM` expressions from row-level join matching. Source/facet selection continues to use the extractor-agnostic `OF` model described below.
