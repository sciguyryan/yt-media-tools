...
`JOIN` grammar is reserved for selected relational composition between independent media relations. A single `INNER JOIN`, `SEMI JOIN` or `ANTI JOIN` is executable. `INNER JOIN` exposes both relations and preserves one result row for every TRUE-matching row pair; existence joins filter the primary relation without multiplying it. `LEFT JOIN` and multi-way JOIN execution remain unsupported. This does not expose internal persistence tables or turn yt-sql into a general relational database.
...
A single `INNER JOIN`, `SEMI JOIN` or `ANTI JOIN` is executable. `INNER JOIN` exposes fields from both relations and emits one row for every row pair whose `ON` predicate evaluates to TRUE, including ordinary duplicate multiplication. `SEMI JOIN` and `ANTI JOIN` use the right relation only for match testing and do not expose its fields or multiply primary-relation rows. For every executable form, FALSE and SQL UNKNOWN do not match. `LEFT JOIN` and multi-way JOIN execution remain behind deterministic execution guards. Semantic scope resolution requires explicit aliases for every participating relation, resolves `alias.field` to a relation-owned field, and rejects ambiguous unqualified fields or invalid aliases. Each `JOIN ... ON` predicate is resolved incrementally: it may reference the primary relation, relations introduced by earlier joins and the right relation introduced by its own edge, but not a relation introduced later. `ON` is a row-level deterministic Boolean expression, so aggregate functions and volatile `RANDOM` expressions are rejected. In a joined query, plain `SELECT *` expands only the primary `FROM` relation. `alias.*` expands only that explicitly named relation, subject to the executable join kind's projection rules. Relation qualification remains semantically distinct from structured postfix member access. CTE and set composition remain separate multi-source composition mechanisms.
