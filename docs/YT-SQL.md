...
`JOIN` grammar is reserved for selected relational composition between independent media relations. A single `INNER JOIN`, `LEFT [OUTER] JOIN`, `SEMI JOIN` or `ANTI JOIN` is executable. `INNER JOIN` exposes both relations and preserves one result row for every TRUE-matching row pair. `LEFT JOIN` preserves the same match multiplicity and additionally emits each unmatched primary-relation row once with the right relation NULL-extended. Existence joins filter the primary relation without multiplying it. Multi-way JOIN execution remains unsupported. This does not expose internal persistence tables or turn yt-sql into a general relational database.
...
A single `INNER JOIN`, `LEFT [OUTER] JOIN`, `SEMI JOIN` or `ANTI JOIN` is executable. `INNER JOIN` exposes fields from both relations and emits one row for every row pair whose `ON` predicate evaluates to TRUE, including ordinary duplicate multiplication. `LEFT JOIN` has the same TRUE-only matching rule and multiplicity, then preserves an unmatched primary row exactly once with every right-relation field evaluating as SQL NULL. This NULL extension is an execution binding rather than a fabricated physical metadata record. `SEMI JOIN` and `ANTI JOIN` use the right relation only for match testing and do not expose its fields or multiply primary-relation rows. For every executable form, FALSE and SQL UNKNOWN do not match. Multi-way JOIN execution remains behind a deterministic execution guard. Semantic scope resolution requires explicit aliases for every participating relation, resolves `alias.field` to a relation-owned field, and rejects ambiguous unqualified fields or invalid aliases. Each `JOIN ... ON` predicate is resolved incrementally: it may reference the primary relation, relations introduced by earlier joins and the right relation introduced by its own edge, but not a relation introduced later. `ON` is a row-level deterministic Boolean expression, so aggregate functions and volatile `RANDOM` expressions are rejected. In a joined query, plain `SELECT *` expands only the primary `FROM` relation. `alias.*` expands only that explicitly named relation, subject to the executable join kind's projection rules. Relation qualification remains semantically distinct from structured postfix member access. JOIN composes with CTE-produced relations and with UNION/UNION ALL branches. Each set branch executes its own relation operator before positional reconciliation, and physical acquisition requirements from ON, projection, filtering and downstream expressions are attributed to the relation that owns each qualified field. Explain output exposes relation identities, join kind, ON dependencies, relation-owned acquisition requirements and the selected relational execution strategy through the same console, JSON and Graphviz explanation architecture used by the rest of the planner.
...
The `raw.*` namespace is deliberately backend-specific. A raw value may be indexed when the observed runtime value is an ordered list or tuple, including a list of structured records or nested ordered collections. In that namespace `[index]` addresses the provider sequence exactly as supplied; it does not promote that order into a portable yt-sql ordering guarantee for the corresponding first-class metadata field. SQL `NULL` and out-of-range behaviour remain unchanged. Mappings, sets, scalar values and dynamically inconsistent values are not made indexable merely because they are iterable or happen to contain collection data in some rows. Raw indexed requirements therefore remain backend-specific and require local collection evaluation unless a backend capability explicitly proves an equivalent indexed acquisition.

Indexed access is also represented explicitly in metadata planning. A direct access such as `tags[0]` records the collection field and constant logical position separately from a request for the whole `tags` collection. A physical backend may satisfy that requirement with a partial indexed acquisition only when its capability contract explicitly guarantees the indexed result is semantically identical to indexing yt-sql's complete logical collection. Dynamic indexes, whole-collection uses, collection-valued expressions that are not direct fields, and backends without that proof require acquisition of the collection needed for local evaluation. Current yt-dlp lowering remains conservative and therefore acquires the complete required collection.
...
First-class format, chapter and thumbnail records use closed yt-sql schemas. Format records expose stable logical members including `format_id`, `ext`, dimensions, resolution, frame rate, aspect ratio, codecs, container/protocol information, dynamic range, bitrates, sample rate, audio channels, exact/approximate filesize, language and format notes. Chapter records expose `title`, `start_time` and `end_time`; thumbnail records expose `id`, `url`, `width` and `height`. These members describe the yt-sql contract rather than promising every extractor supplies every value, so extractor-dependent members are nullable. Provider fields outside those declared schemas do not become first-class members automatically.

Structured collections remain subject to their collection ordering contract. Declaring the members of `formats`, `chapters` or `thumbnails` does not make their backend list order a yt-sql positional guarantee, so an expression such as `formats[0].height` remains invalid while that collection's logical ordering is unknown. Member access can nevertheless compose with any structured value produced through a semantically valid expression.

Dynamic `raw.*` dictionaries use a separate conservative schema-inference boundary. yt-sql exposes only string-keyed members whose observed values have one compatible runtime shape; missing members become nullable, nested compatible dictionaries may themselves become dynamic structured values, and incompatible members are omitted. If a raw record cannot be described safely it remains opaque. Whole structured raw values are still not ordinary selectable scalar values. When a raw structured record is addressed directly, parentheses make postfix member access explicit and avoid ambiguity with the established dotted raw-field path syntax: `(raw.provider_record).provider_id`. Ordered raw record sequences may combine indexing and member access naturally, for example `raw.provider_formats[1].height`.

```text
SELECT id, (raw.provider_record).provider_id AS provider_id FROM @example
SELECT id, (raw.provider_record).dimensions.height AS height FROM @example
SELECT id, raw.provider_formats[1].height AS second_height FROM @example
SELECT id FROM @example WHERE (raw.provider_record).provider_id = 'primary'
```
