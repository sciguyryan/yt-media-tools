...
`JOIN` grammar is reserved for selected relational composition between independent media relations. A single `SEMI JOIN` or `ANTI JOIN` is executable as an existence filter over the primary relation. `INNER JOIN`, `LEFT JOIN` and multi-way JOIN execution remain unsupported. This does not expose internal persistence tables or turn yt-sql into a general relational database.
