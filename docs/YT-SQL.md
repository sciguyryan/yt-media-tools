...
`JOIN` execution remains intentionally unsupported at this stage. Its accepted grammar and AST are present so later relational phases can add semantics without parser churn. Semantic scope resolution requires explicit aliases for every participating relation, resolves `alias.field` to a relation-owned field, and rejects ambiguous unqualified fields or invalid aliases before reaching the fail-closed execution boundary. Each `JOIN ... ON` predicate is resolved incrementally: it may reference the primary relation, relations introduced by earlier joins and the right relation introduced by its own edge, but not a relation introduced later. `ON` is a row-level deterministic Boolean expression, so aggregate functions and volatile `RANDOM` expressions are rejected. In a joined query, plain `SELECT *` expands only the primary `FROM` relation. `alias.*` expands only that explicitly named relation and may be combined with other qualified projections. Joined projections never flatten right-side fields implicitly, and duplicate output names are rejected unless explicit `AS` aliases make them unique. Relation qualification remains semantically distinct from structured postfix member access. CTE and set composition continue to provide the currently executable multi-source composition model.
