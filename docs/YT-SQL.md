...
Discover recognises ordered scalar list and tuple metadata as stable positional collections. Set-valued metadata is treated as unordered and therefore cannot be indexed positionally. Dynamic `raw.*` paths may be indexed when their runtime values form ordered scalar collections. Structured record collections remain outside this phase and continue to require the separate structured-value/member-access work.
