...
- Add `--query-file FILE` for reusable UTF-8 yt-sql queries while preserving the established query-processing and parameter-binding path.
- Add backend source resolution and capability-oriented metadata-provider selection without weakening logical source identity or yt-sql semantics.
- Add bounded external-tool diagnostics and comparable metadata-provider benchmark infrastructure used to evaluate alternative detailed-metadata acquisition paths.
- Investigate YouTube.js, youtube-innertube, pytubefix, NewPipeExtractor, Invidious, Piped and ytmusicapi against explicit capability, authority, reliability and trust boundaries.
- Promote a conservative YouTube.js `getBasicInfo()` exact-scalar capability for resolved YouTube sources, with batched execution, session reuse, provenance and yt-dlp fallback.
